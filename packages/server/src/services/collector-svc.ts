...
function get(username: string): Promise<{
  ...
  return CollectorModel.findOne({ username }).then(
    async (collector: { shoeCollection: string | any[]; username: any }) => {
      if (!collector) {
      ...
      const totalValue = shoes.reduce(
        (sum: any, shoe: { price: { marketPrice: any } }) => {
      ...
      const brandCount = shoes.reduce(
        (acc: Record<string, number>, shoe: { brand: string | number }) => {
      ...
      const manufacturers = Array.from(
        new Set(shoes.map((shoe: { brand: string }) => shoe.brand))
      );
